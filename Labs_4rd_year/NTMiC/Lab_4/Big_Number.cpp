...
Big_Number Big_Number::operator%(const Big_Number &num) const
{
    ...
    while (j > -1)
    {
        DBASE q = (DBASE)((DBASE)((DBASE)((DBASE)(newNum.digits[j + delNum.length]) * (DBASE)(b)) +
                                  (DBASE)(newNum.digits[j + delNum.length - 1])) /
                          (DBASE)(delNum.digits[delNum.length - 1]));
...
Big_Number &Big_Number::operator/=(const Big_Number &divisor)
{
    ...
    while (j > -1)
    {
        DBASE q = (DBASE)((DBASE)((DBASE)((DBASE)(dividend.digits[j + divisor_normalized.length]) * (DBASE)(base_val)) +
                                  (DBASE)(dividend.digits[j + divisor_normalized.length - 1])) /
                          (DBASE)(divisor_normalized.digits[divisor_normalized.length - 1]));
...
Big_Number Big_Number::random(int bits)
{
    ...
    {
        rand_num.digits[i] = (static_cast<unsigned long long>(rand()) << 32) | static_cast<unsigned long long>(rand());
    }

    // Mask the higher bits in the top limb if necessary
    int extra_bits = bits % BASE_SIZE;
    if (extra_bits != 0) {
        unsigned long long mask = (1ULL << extra_bits) - 1ULL;
        rand_num.digits[num_limbs - 1] &= mask;
    }
...
Big_Number Big_Number::random_range(const Big_Number &min, const Big_Number &max)
{
    ...
    Big_Number range = max - min + Big_Number(1ULL);
    int approx_bits = max.length * BASE_SIZE; // Approximate bit length
    Big_Number rand_val;
    do {
        rand_val = random(approx_bits);
...
Big_Number Big_Number::nth_sqrt(const Big_Number &n) const
{
    Big_Number x_previous = *this;
    Big_Number x_next = *this;
    Big_Number a = *this;
    Big_Number one = Big_Number(1ULL);
    Big_Number n_minus_1 = n - one;
    do
    {
        x_previous = x_next;
        // Используем x_previous, а не x_next!
        Big_Number x_power = x_previous.pow(n_minus_1);
        Big_Number A_div_power = a / x_power;
        Big_Number numerator = n_minus_1 * x_previous + A_div_power;

        // Вычисляем новое значение
        x_next = numerator / n;
    } while (x_next < x_previous);
    return x_previous;
}

Big_Number Big_Number::method_Olvea()
{
    Big_Number zero(0ULL);
    Big_Number one(1ULL);
    Big_Number two(2ULL);
    Big_Number three(3ULL);
    Big_Number four(4ULL);
    Big_Number number = *this;

    while (number % two == zero)
    {
        number /= two;
    }
    if (number == one)
    ...
    Big_Number s = number.sqrt();
    Big_Number cube_root = number.nth_sqrt(three);
    Big_Number divider = (two * cube_root) + one;

    Big_Number bord_left = divider;
    Big_Number bord_right = s;    

    Big_Number divider_minus_2 = (divider >= two) ? divider - two : one;
    Big_Number r1 = number % divider;
    Big_Number r2 = number % divider_minus_2;
    Big_Number q = four * ((number / divider_minus_2) - (number / divider));
    while (true)
    {
        divider += two;
        if (divider > s)
        {
            std::cout << "No solution in range:" << std::endl;
            bord_left.cout_10();
            bord_right.cout_10();
            return Big_Number(0ULL);
        }
        Big_Number temp = two * r1 + q;
        Big_Number r;
        if (temp >= r2)
        {
            r = temp - r2;
...
std::vector<Big_Number> Big_Number::method_Ferma() {
    std::vector<Big_Number> result;
    Big_Number number = *this;
    Big_Number z;
    Big_Number y;
    Big_Number x = number.sqrt();
    if (x* x == number) {
        result.push_back(x);
        result.push_back(x);
        return result;
    };
    Big_Number zPrev = (x + 1) * (x + 1) - number;
    while (true) {
        x += Big_Number(1ULL);

        Big_Number temp = (number + 1) / Big_Number(2ULL);
        if (temp == x) {
            return result;
        }
        z = zPrev;
        ...
        zPrev = z + (Big_Number(2ULL) * (x + 1) - 1); 
    }
}

Big_Number Big_Number::algorithm_evklida(const Big_Number & number_2) const {
    Big_Number a = *this;
    Big_Number b = number_2;
    Big_Number zero(0ULL);
    
    // Проверка на нулевые значения
    if (a == zero && b == zero) {
        throw invalid_argument("Both numbers cannot be zero");
    }
    if (a == zero) return b;
    if (b == zero) return a;
...
Big_Number Big_Number::p_method_polarda_factorize() const{
    Big_Number number = * this;
    Big_Number a = Big_Number(2ULL);
    Big_Number b = Big_Number(2ULL);
    Big_Number d;
    Big_Number temp;
    while (true) {
        a = (a * a + Big_Number(1ULL)) % number;
        temp =  (b * b + Big_Number(1ULL)) % number;
        b = (temp * temp + Big_Number(1ULL)) % number;
        if (a == b) {
            return Big_Number(0ULL);
        }
        if (a > b) {
        ...
        d = temp.algorithm_evklida(number);
        if (d != Big_Number(1ULL)) {
            return d;
        }
    }
}

Big_Number Big_Number::log2() const {
    BASE mask = (1 << (BASE_SIZE - 1));
    BASE result = BASE_SIZE * this->length;
    BASE q = (mask&this->digits[this->length - 1]);
    while (!q)
    {
        mask = mask >> 1;
        result--;
        q = (mask&this->digits[this->length - 1]);
    }
    Big_Number res;
    res.digits[0] = result;
    return res;
...
int Big_Number::find_highest_bit_position() const {
    // Ищем в старшем разряде
    Big_Number x = *this;
    for (int i =x.length; i >= 0; --i) {
        if (x.digits[i] != 0) {
            BASE digit = x.digits[i];
            // Находим старший установленный бит в этом разряде
            for (int bit = BASE_SIZE - 1; bit >= 0; --bit) {
                if (digit & (1ULL << bit)) {
                    return i * BASE_SIZE + bit;
                }
            }
        }
    }
    return 0; // если число 0
}

Big_Number Big_Number::p_minus_1_method_polarda_factorize() const {
    Big_Number number = *this;
    Big_Number d;
    Big_Number a;
    Big_Number one(1ULL);
    Big_Number two(2ULL);
    Big_Number B = Big_Number(100ULL);
    Big_Number e;
    std::vector<Big_Number> primes = {
        Big_Number(2ULL), Big_Number(3ULL), Big_Number(5ULL), Big_Number(7ULL),
        Big_Number(11ULL), Big_Number(13ULL), Big_Number(17ULL), Big_Number(19ULL),
        Big_Number(23ULL), Big_Number(29ULL), Big_Number(31ULL), Big_Number(37ULL),
        Big_Number(41ULL), Big_Number(43ULL), Big_Number(47ULL), Big_Number(53ULL),
        Big_Number(59ULL), Big_Number(61ULL), Big_Number(67ULL), Big_Number(71ULL),
        Big_Number(73ULL), Big_Number(79ULL), Big_Number(83ULL), Big_Number(89ULL),
        Big_Number(97ULL)
    };
    a = random_range(two, number - two);
    std::cout << "A: ";
    a.cout_10();
    d = a.algorithm_evklida(number);
    if (d > Big_Number(1ULL)) {
        return d;
    }
    Big_Number current = a;
    ...
    for (int j = 0; j < 10; j++)
    {
        ...
        for (int i = 0; i < primes.size(); i++) {
            e = ((number.log2())/ primes[i].log2());
            a = a.pow_mod(primes[i].pow(e), number);
        }
        if (a == Big_Number(1ULL)) {
            std::cout << "A: ";
            current = random_range(two, number - two);
                        current.cout_10();
            continue;
        }
        a.cout_10();
        d = (a - Big_Number(1ULL)).algorithm_evklida(number);
        if (d == Big_Number(1ULL)) {
            return Big_Number(0ULL);
        }
        else {

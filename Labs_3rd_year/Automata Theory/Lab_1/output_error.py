...
class CustomFTPHandler(FTPHandler):
    ...
    def ftp_SIZE(self, filename):
        if os.path.exists(filename):
            ...
            self.respond("333 You found me")  # Ответ на ошибку, если файл не существует
...
def run_ftp_server():
    ...
    # Запуск FTP-сервера
    server = FTPServer(("127.0.0.2", 2121), handler)  # Используем порт 2121 для тестов
...
def test_ftp_server(test_cases):
    ...
    ftp.connect("127.0.0.2", 2121, timeout=300)  # Подключаемся к порту 2121
    for i in range(len(test_cases)):
        ...
        ftp.connect("127.0.0.2", 2121)  # Переподключаемся
    ftp.close()
...
def write_to_files(results):
    with open("results_output.txt", "w", encoding="utf-8") as result_file:
        for item in results:
...
if __name__ == "__main__":
    ...
    with open("test_results_error_output.txt", "w", encoding="utf-8") as test_result_file:

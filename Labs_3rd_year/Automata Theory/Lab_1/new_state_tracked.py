...
class CustomFTPHandler(FTPHandler):
    ...
    def ftp_PASS(self, password):
        ...
        if self.state == "Error":
            self.respond("333 You found me")
        elif not self.authenticated and not self.username:
            self.respond("503 Login with USER first")
        elif self.authenticated and password == "correct_pass":
            self.respond("503 Already logged")
        elif self.authenticated and self.state == "Error" and password == "error_pass":
            self.respond("333 You found me")
        elif self.authenticated and password == "error_pass":
            self.respond("503 Already logged")
    ...
    def ftp_SIZE(self, filename):
        if self.state == "Error":
            self.respond("333 You found me")
        elif self.state == "Error" and filename == "correct_file.txt":
            super().ftp_SIZE(filename)
        elif os.path.exists(filename):
            file_size = os.path.getsize(filename)
            self.respond(f"213 {file_size}")  # Код ответа 213: размер файла
        else:
            self.respond("550 You found me")  # Ответ на ошибку, если файл не существует
            self.state = "Error"
